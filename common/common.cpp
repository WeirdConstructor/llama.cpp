...
std::vector<llama_token> llama_tokenize(
  const struct llama_context * ctx,
           const std::string & text,
                        ...
                        bool   parse_special,
                        bool   mid_piece) {
    return llama_tokenize(llama_get_model(ctx), text, add_special, parse_special, mid_piece);
}
...
std::vector<llama_token> llama_tokenize(
    ...
    if (mid_piece) {
        n_tokens = llama_tokenize_mid_piece(model, text.data(), text.length(), result.data(), result.size(), add_special, parse_special);
    } else {
    ...
    if (n_tokens < 0) {
        ...
        int check = 0;
        if (mid_piece) {
            check = llama_tokenize_mid_piece(model, text.data(), text.length(), result.data(), result.size(), add_special, parse_special);
        } else {
            check = llama_tokenize(model, text.data(), text.length(), result.data(), result.size(), add_special, parse_special);
        }
